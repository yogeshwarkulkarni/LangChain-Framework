...
var ReactAgent = class ReactAgent {
	...
	constructor(options, defaultConfig) {
		...
		*/
		...
		const workflow = new __langchain_langgraph.StateGraph(state, {
			input,
			output,
		...
		*/
		...
		for (let i = 0; i < beforeAgentNodes.length; i++) {
			...
			if (node.allowed && node.allowed.length > 0) {
				const allowedMapped = node.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== require_ToolNode.TOOLS_NODE_NAME || hasToolsAvailable);
		...
		if (afterModelNodes.length > 0) {
			...
			const modelPaths = this.#getModelPaths(clientTools, true, hasToolsAvailable).filter((p) => p !== require_ToolNode.TOOLS_NODE_NAME || hasToolsAvailable);
		...
		if (afterAgentNodes.length > 0) {
			...
			if (firstAfterAgent.allowed && firstAfterAgent.allowed.length > 0) {
				const allowedMapped = firstAfterAgent.allowed.map((t) => require_utils$1.parseJumpToTarget(t)).filter((dest) => dest !== require_ToolNode.TOOLS_NODE_NAME || hasToolsAvailable);
				/**
				...
				*/
				...
				allNodeWorkflows.addConditionalEdges(firstAfterAgentNode, this.#createAfterModelSequenceRouter(clientTools, firstAfterAgent.allowed, __langchain_langgraph.END, hasToolsAvailable), destinations);
	...
	*/
	withConfig(config) {
		return new ReactAgent(this.options, (0, __langchain_core_runnables.mergeConfigs)(this.#defaultConfig, config));
	}
	/**
		...
		return (state) => {
			...
			if (builtInState.jumpTo) {
				...
				if (dest === require_ToolNode.TOOLS_NODE_NAME && allowedSet.has(require_ToolNode.TOOLS_NODE_NAME)) {
					if (!hasToolsAvailable) return __langchain_langgraph.END;
					return new __langchain_langgraph.Send(require_ToolNode.TOOLS_NODE_NAME, {
						...state,
						jumpTo: void 0
					});
 ...
 /**
	...
	*/
	async invoke(state, config) {
		const mergedConfig = (0, __langchain_core_runnables.mergeConfigs)(this.#defaultConfig, config);
